...
namespace esimd
{
  ...
  /*`QuaternionT(yaw, pitch, roll)` is Y-up and right-handed — yaw about Y, pitch about X,
  roll about Z — composed as `Qy(yaw) * Qx(pitch) * Qz(roll)`, so despite the argument
  order the fixed-frame order is roll, then pitch, then yaw.*/
